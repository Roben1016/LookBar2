...
public class BlinkLayout extends LinearLayout {
    ...
    public BlinkLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(
                attrs,
                R.styleable.page_BlinkLayout,
                0, 0);

        try {
            shimmerAnimationDuration = a.getInteger(R.styleable.page_BlinkLayout_page_blink_animation_duration, 3000);
            shimmerColor = a.getColor(R.styleable.page_BlinkLayout_page_blink_color, ContextCompat.getColor(context, R.color.page_shimmer_color));
        } finally {
            a.recycle();
        }
    }
